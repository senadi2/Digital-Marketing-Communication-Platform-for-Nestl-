...
const campaignTableHead = document.getElementById("campaignTableHead");
...
function renderCampaignTableHead(showStatusColumn) {
    campaignTableHead.innerHTML = `
        <tr>
            <th>Campaign</th>
            <th>Product</th>
            <th>Agency</th>
            ${showStatusColumn ? "<th>Review Status</th>" : ""}
            <th>Latest Upload</th>
        </tr>
    `;
}
...
function renderCampaignTable() {
    const showStatusColumn = activeFilter === "All";
    const columnCount = showStatusColumn ? 5 : 4;
    renderCampaignTableHead(showStatusColumn);
    ...
    if (!visible.length) {
        campaignTableBody.innerHTML = `<tr><td colspan="${columnCount}"><div class="empty-state">No campaigns found for this review status.</div></td></tr>`;
        return;
    }
    ...
    visible.forEach((campaign) => {
        ...
        row.innerHTML = `
            ...
            ${showStatusColumn ? `<td><span class="status-pill ${escapeHtml(statusClass(state.status))}">${escapeHtml(state.status)}</span></td>` : ""}
...
async function loadReviewOverview() {
    try {
        ...
        const columnCount = activeFilter === "All" ? 5 : 4;
        renderCampaignTableHead(activeFilter === "All");
        campaignTableBody.innerHTML = `<tr><td colspan="${columnCount}"><div class="empty-state">${escapeHtml(err.message || "Could not load brand review overview.")}</div></td></tr>`;
    }
}
